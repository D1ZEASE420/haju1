...
function deleteProduct(id) {
    if (!confirm('Kas soovid selle toote kustutada?')) return;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
            ...
            <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                    ...
                    <div class="relative aspect-[4/3] overflow-hidden bg-muted">
                        ...
                        <!-- Out of stock badge -->
                        <span v-if="!product.in_stock" class="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-destructive/90 text-white text-xs font-medium">
                            Otsas
                        </span>
                        ...
                        >
                            <Link
                                ...
                                class="h-8 w-8 flex items-center justify-center rounded-lg bg-background/90 backdrop-blur-sm border border-border/50 text-muted-foreground hover:text-foreground transition-colors"
                                title="Muuda toodet"
                            >
                            ...
                            <button
                                ...
                                class="h-8 w-8 flex items-center justify-center rounded-lg bg-background/90 backdrop-blur-sm border border-destructive/40 text-destructive hover:bg-destructive/10 transition-colors"
                                title="Kustuta toode"
                            >
                                <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="flex flex-col flex-1 p-5 gap-3">
                        ...
                        <div class="pt-3 border-t border-border">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <div v-if="product.in_stock" class="flex items-center gap-1 border border-border rounded-lg overflow-hidden">
                                    <button
                            ...
                            <div v-else class="w-full py-2.5 rounded-xl bg-muted text-muted-foreground text-sm font-medium text-center">
                                Laost otsas
                            </div>
                        </div>
                    </div>
        ...
        <Teleport to="body">
            <Transition name="fade">
                <div v-if="cartOpen" @click="cartOpen = false" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-40"></div>
            </Transition>

            <Transition name="slide-right">
                <div v-if="cartOpen" class="fixed right-0 top-0 bottom-0 w-full max-w-sm bg-card border-l border-border shadow-2xl z-50 flex flex-col">
                    <div class="flex items-center justify-between px-6 py-5 border-b border-border shrink-0">
                        ...
                        <div class="flex items-center gap-2">
                            <button v-if="cart.length > 0" @click="clearCart" class="text-xs text-muted-foreground hover:text-destructive transition-colors">Tühjenda</button>
                            <button @click="cartOpen = false" class="p-1.5 rounded-lg hover:bg-accent text-muted-foreground hover:text-foreground transition-colors">
                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="flex-1 overflow-y-auto px-6 py-4">
                        ...
                        <div v-else class="flex flex-col gap-4">
                            <div v-for="item in cart" :key="item.id" class="flex gap-3 p-3 rounded-xl border border-border bg-background">
                                <img :src="item.image_url" :alt="item.name" class="h-16 w-16 rounded-lg object-cover shrink-0"/>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 mt-2">
                                        <div class="flex items-center gap-1 border border-border rounded-lg overflow-hidden">
                                            <button @click="updateQty(item.id, item.quantity - 1)" class="w-7 h-7 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent text-sm transition-colors">−</button>
                                            <span class="w-7 text-center text-sm font-medium text-foreground">{{ item.quantity }}</span>
                                            <button @click="updateQty(item.id, item.quantity + 1)" class="w-7 h-7 flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent text-sm transition-colors">+</button>
                                        </div>
                                        <span class="text-xs text-muted-foreground ml-auto">{{ (item.price * item.quantity).toFixed(2) }} €</span>
                                        <button @click="removeItem(item.id)" class="p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
                    ...
                    <div v-if="cart.length > 0" class="px-6 py-5 border-t border-border shrink-0 space-y-4">
                        ...
                        <Link href="/checkout" class="block w-full py-3 rounded-xl bg-primary text-primary-foreground text-sm font-medium text-center hover:bg-primary/90 active:scale-[0.98] transition-all shadow-sm shadow-primary/25">
                            Jätka kassasse →
                        </Link>
                    </div>
                </div>
            ...
            <Transition name="toast">
                <div
                    ...
                    :class="[
                        ...
                        toast.type === 'error' ? 'bg-destructive/10 border-destructive/30 text-destructive' : 'bg-card border-border text-foreground'
                    ]"
                >
